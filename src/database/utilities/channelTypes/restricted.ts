...
export const setChannelType = async (
  channelId: string,
  guildId: string,
  kind: string
) => {
...
export const removeChannelType = async (
  channelId: string,
  guildId: string,
  kind: string
) => {
  ...
  settings.guildId = guildId;
  if (kind === 'user')
    if (settings.userOnly)
      settings.userOnly = settings.userOnly.filter((c) => c !== channelId);
  if (kind === 'suggestions')
    if (settings.suggestions)
      settings.suggestions = settings.suggestions.filter(
        (c) => c !== channelId
      );
